...
- **Обновленный `/api/works`** - поддержка выбора метода оплаты
- **Расширенный `/api/junior/paypal`** - управление PayPal аккаунтами с сортировкой
...
## 🔄 Обновления v2.1 (Сентябрь 2025)

### Реорганизация интерфейса Junior:
**Навигация оптимизирована с 11 до 9 пунктов:**
- ✅ **Объединены**: PayPal аккаунты + PayPal работы → "Платежные средства"
- ✅ **Объединены**: Карты + PayPal → единая страница с табами
- ✅ **Переименованы**: "Мои выводы" → "Мои работы"
- ✅ **Добавлен**: Рейтинг с лидербордом и мотивацией
- ✅ **Добавлен**: Инструменты (генератор + форматирование)
- ✅ **Удалены дублирующие страницы**: `/dashboard/junior/paypal` и `/dashboard/junior/paypal-work`
- ✅ **Исправлен редирект**: "Добавить PayPal аккаунт" теперь ведет на `/dashboard/junior/payment-methods?tab=paypal`
- ✅ **Валюта по умолчанию**: Изменена с USD на GBP (£) для PayPal аккаунтов

### Интеграция PayPal в систему работ:
- ✅ **Выбор метода оплаты** при создании работы (карта/PayPal)
- ✅ **Автоматическая сортировка** PayPal по балансу
- ✅ **Превью работы** перед созданием
- ✅ **API `/api/paypal-works`** для PayPal операций

### Система месячных циклов:
- ✅ **Активные средства** - для текущего месяца
- ✅ **История** - заблокированные/неактивные за прошлые месяцы
- ✅ **Приоритизация** средств с балансом
