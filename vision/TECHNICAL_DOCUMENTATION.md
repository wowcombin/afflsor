# Техническая документация Afflsor ERP System v2.1
...
- **PayPal**: автоматическая сортировка по балансу с фильтрацией
  - 💰 С балансом (сортировка по убыванию) - готовы к работе
  - ⚠️ Без баланса - требуют пополнения
  - 🚫 Заблокированные - в истории с возможностью восстановления
  - **Фильтры**: "Все аккаунты" / "С балансом" / "Без баланса"
  - **Добавление PayPal**: валюта по умолчанию GBP (£) с возможностью выбора USD/EUR/CAD
  - **Отображение валют**: £ (GBP), € (EUR), C$ (CAD), $ (USD) вместо универсального $
  - **Действия через иконки**:
    - ✏️ **Редактирование**: изменение данных аккаунта, включая баланс и валюту
    - 💼 **Создать работу**: быстрое создание работы с выбранным PayPal
    - 📄 **Операции**: запись и отслеживание переводов (отправка/получение денег, выводы на карту, депозиты в казино)
    - 🚫 **Заблокировать**: перевод аккаунта в статус "blocked"
    - ↩️ **Вернуть**: восстановление заблокированного аккаунта в активный статус
...
**Рейтинг и мотивация** (`/dashboard/junior/leaderboard`):
...
- `/dashboard/teamlead/withdrawals` - Проверка выводов команды
- `/dashboard/teamlead/casino-assignments` - Назначение Junior к казино
- `/dashboard/teamlead/banks` - Управление назначенными банками
- `/dashboard/teamlead/cards` - Управление картами команды
- `/dashboard/teamlead/tools` - Инструменты (генератор + форматирование)
- `/dashboard/teamlead/kanban` - Kanban доска задач
- `/dashboard/teamlead/tasks` - Задачи команды

#### Функциональность:
- Управление назначенными Junior
- Проверка и одобрение выводов своей команды
- Назначение Junior к казино (только к назначенным TeamLead)
- Выпуск и назначение "розовых" карт на основе назначенных банков
- Координация работы команды
- Получение 10% от профита команды
- Автоматическое создание индивидуальных команд

**Инструменты** (`/dashboard/teamlead/tools`):
- **Генератор британских аккаунтов**: полные данные (username, password, email, phone)
- **Британские номера телефонов**: префиксы реальных операторов (O2, Vodafone, EE, Three)
- **Email адреса**: популярные британские домены (gmail.com, yahoo.co.uk, btinternet.com)
- **Форматирование текста**: преобразование в нижний регистр
- **Пользовательские имена**: возможность задать собственные имена для генерации
- **Быстрое копирование**: одним кликом в буфер обмена
