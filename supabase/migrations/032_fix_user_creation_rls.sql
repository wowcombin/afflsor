...
CREATE POLICY "HR Manager Admin могут создавать пользователей" ON users
...
CREATE POLICY "HR Manager Admin могут изменять пользователей" ON users
    FOR UPDATE USING (
        auth.uid() IN (
            SELECT auth_id FROM users 
            WHERE role IN ('hr', 'admin', 'manager') AND status = 'active'
...
CREATE POLICY "HR Manager Admin могут видеть всех пользователей" ON users
    FOR SELECT USING (
        auth.uid() IN (
            SELECT auth_id FROM users 
            WHERE role IN ('hr', 'admin', 'manager') AND status = 'active'
