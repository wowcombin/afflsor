-- Интеграция PayPal работ в расчет зарплат
-- Миграция 040: PayPal в расчете зарплат

-- Обновляем функцию расчета зарплат для включения PayPal работ
CREATE OR REPLACE FUNCTION calculate_monthly_salaries_v3(
  p_month VARCHAR(7),
...
DECLARE
  total_all DECIMAL(10,2); -- ALL = сумма всех профитов джунов + профит Manual QA (включая PayPal)
...
BEGIN
  -- 1. Получаем ALL = сумма всех профитов джунов + профит Manual QA (включая PayPal)
  WITH junior_profits AS (
    SELECT 
      u.id,
      -- Профит с обычных карт
      COALESCE(SUM(ww.withdrawal_amount - w.deposit_amount), 0) +
      -- Профит с PayPal работ
      COALESCE(SUM(pw.withdrawal_amount - pw_work.deposit_amount), 0) as profit
    FROM users u
    -- Обычные работы с картами
    ...
    -- PayPal работы
    LEFT JOIN paypal_works pw_work ON pw_work.user_id = u.id 
      AND DATE_TRUNC('month', pw_work.created_at) = (p_month || '-01')::DATE
      AND pw_work.status = 'completed'
    LEFT JOIN paypal_withdrawals pw ON pw.paypal_work_id = pw_work.id 
  ...
  -- 5. Определяем лидера месяца (самый крупный депозит среди всех Junior, включая PayPal)
  SELECT 
    u.id as employee_id,
    MAX(GREATEST(COALESCE(w.deposit_amount, 0), COALESCE(pw_work.deposit_amount, 0))) as largest_profit
  INTO leader_record
  FROM users u
  LEFT JOIN works w ON w.junior_id = u.id
    AND DATE_TRUNC('month', w.created_at) = (p_month || '-01')::DATE
  ...
  WHERE u.role = 'junior'
    AND u.status = 'active'
    AND (w.id IS NOT NULL OR pw_work.id IS NOT NULL)
  GROUP BY u.id
  ...
  WITH employee_profits AS (
    SELECT 
      ...
      u.role,
      -- Индивидуальный профит сотрудника (карты + PayPal)
      CASE 
        WHEN u.role IN ('junior', 'tester') THEN
          -- Профит с обычных карт
          COALESCE(SUM(ww.withdrawal_amount - w.deposit_amount), 0) +
          -- Профит с PayPal работ
          COALESCE(SUM(pw.withdrawal_amount - pw_work.deposit_amount), 0)
        ELSE 0
      END as individual_profit,
      -- Профит команды для Team Lead (карты + PayPal)
      CASE 
        WHEN u.role = 'teamlead' THEN
          COALESCE((
            SELECT 
              SUM(COALESCE(ww2.withdrawal_amount - w2.deposit_amount, 0)) +
              SUM(COALESCE(pw2.withdrawal_amount - pw_work2.deposit_amount, 0))
            FROM users u2
            -- Обычные работы команды
            LEFT JOIN works w2 ON w2.user_id = u2.id
            ...
            -- PayPal работы команды
            LEFT JOIN paypal_works pw_work2 ON pw_work2.user_id = u2.id
              AND DATE_TRUNC('month', pw_work2.created_at) = (p_month || '-01')::DATE
              AND pw_work2.status = 'completed'
            LEFT JOIN paypal_withdrawals pw2 ON pw2.paypal_work_id = pw_work2.id
              AND pw2.status = 'completed'
  ...
  salary_calculations AS (
    SELECT 
      ...
      -- Расчет базовой зарплаты по ролям (включая PayPal профиты)
      CASE 
        -- Junior: 10% от своего профита (карты + PayPal)
        WHEN ep.role = 'junior' THEN 
          (ep.individual_profit * 0.10)
        -- Team Lead: 10% от профита команды (карты + PayPal)
        ...
        ELSE 0
      END as base_sal,
      -- Бонусы для Junior (включая PayPal профиты)
      CASE 
        WHEN ep.role = 'junior' THEN
        ...
        ELSE 0.00
      END as perf_bonus,
      -- Лидерский бонус (10% от самого большого депозита, включая PayPal)
      CASE 
  ...
  -- Расчет для CEO (остаток после всех выплат, включая PayPal)
  SELECT 
      ...
      total_all - total_expenses_amount - (
        ...
        FROM (
          SELECT 
            CASE 
              WHEN u.role = 'junior' THEN 
                (COALESCE(combined_profits.profit, 0) * 0.10) +
                CASE WHEN COALESCE(combined_profits.profit, 0) >= 2000 THEN 200.00 ELSE 0.00 END +
                CASE WHEN COALESCE(combined_profits.profit, 0) > 7000 THEN (COALESCE(combined_profits.profit, 0) - 7000) * 0.20 ELSE 0.00 END +
                CASE WHEN u.id = leader_record.employee_id THEN (leader_record.largest_profit * 0.10) ELSE 0.00 END
              WHEN u.role = 'teamlead' THEN 
                COALESCE(team_combined_profits.profit, 0) * 0.10
          ...
          LEFT JOIN (
            SELECT 
              u2.id,
              -- Комбинированный профит: карты + PayPal
              COALESCE(SUM(ww.withdrawal_amount - w.deposit_amount), 0) +
              COALESCE(SUM(pw.withdrawal_amount - pw_work.deposit_amount), 0) as profit
            FROM users u2
            ...
            LEFT JOIN work_withdrawals ww ON ww.work_id = w.id 
              AND ww.status = 'received'
            LEFT JOIN paypal_works pw_work ON pw_work.user_id = u2.id 
              AND DATE_TRUNC('month', pw_work.created_at) = (p_month || '-01')::DATE
              AND pw_work.status = 'completed'
            LEFT JOIN paypal_withdrawals pw ON pw.paypal_work_id = pw_work.id 
              AND pw.status = 'completed'
            ...
            GROUP BY u2.id
          ) combined_profits ON combined_profits.id = u.id
          LEFT JOIN (
            SELECT 
              tl.id,
              -- Комбинированный профит команды: карты + PayPal
              COALESCE(SUM(ww2.withdrawal_amount - w2.deposit_amount), 0) +
              COALESCE(SUM(pw2.withdrawal_amount - pw_work2.deposit_amount), 0) as profit
            FROM users tl
            ...
            LEFT JOIN work_withdrawals ww2 ON ww2.work_id = w2.id 
              AND ww2.status = 'received'
            LEFT JOIN paypal_works pw_work2 ON pw_work2.user_id = j.id 
              AND DATE_TRUNC('month', pw_work2.created_at) = (p_month || '-01')::DATE
              AND pw_work2.status = 'completed'
            LEFT JOIN paypal_withdrawals pw2 ON pw2.paypal_work_id = pw_work2.id 
              AND pw2.status = 'completed'
            ...
            GROUP BY tl.id
          ) team_combined_profits ON team_combined_profits.id = u.id
    ...
    'Остаток после всех выплат и расходов (включая PayPal)'::TEXT as notes
...
-- Обновляем представление для отображения новой формулы
CREATE OR REPLACE VIEW salary_summary_v3 AS
SELECT 
  u.id,
  u.email,
  ...
  u.salary_bonus,
  CASE 
    WHEN u.role = 'junior' THEN 'Профит (карты + PayPal) × 10% + бонусы'
    WHEN u.role = 'teamlead' THEN 'Профит команды (карты + PayPal) × 10%'
...
-- Права доступа
GRANT EXECUTE ON FUNCTION calculate_monthly_salaries_v3 TO authenticated;
GRANT SELECT ON salary_summary_v3 TO authenticated;

-- Комментарии
COMMENT ON FUNCTION calculate_monthly_salaries_v3 IS 'Расчет зарплат с учетом PayPal работ (карты + PayPal профиты)';
COMMENT ON VIEW salary_summary_v3 IS 'Сводка по формулам расчета зарплат включая PayPal работы';

-- Создаем индексы для PayPal таблиц для производительности
CREATE INDEX IF NOT EXISTS idx_paypal_works_user_created_status ON paypal_works(user_id, created_at, status);
CREATE INDEX IF NOT EXISTS idx_paypal_withdrawals_work_status ON paypal_withdrawals(paypal_work_id, status);
