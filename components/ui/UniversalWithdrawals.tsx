...
function UniversalWithdrawalsPage({ userRole }: UniversalWithdrawalsPageProps) {
  ...
  const columns: Column<Withdrawal>[] = [
    ...
    },
    {
      ...
      label: 'Сумма',
      render: (withdrawal) => {
        const usdAmount = Number(convertToUSD(Number(withdrawal.amount), withdrawal.currency || 'USD'))
        const profit = usdAmount - Number(convertToUSD(Number(withdrawal.deposit_amount), withdrawal.currency || 'USD'))
        
        return (
          <div>
            <div className="font-medium text-gray-900">
              {Number(withdrawal.amount).toFixed(2)} {withdrawal.currency || 'USD'}
            </div>
